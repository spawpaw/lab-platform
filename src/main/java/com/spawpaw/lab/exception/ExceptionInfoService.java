...
import com.spawpaw.lab.common.controller.CommonResponse;
...
public class ExceptionInfoService {
    ...
    ExceptionInfoService() {
        ...
        registerEx(10002, "密码错误", WrongPasswordException.class.toString());
        registerEx(10002, "没哟组找到用户", UserNotFoundException.class.toString());
    ...
    public <T> CommonResponse getCommonResponse(T e) {
        if (errorMap.containsKey(e.getClass().toString())) {
            CommonResponse response = errorMap.get(e.getClass().toString());
            if (((Exception) e).getMessage() != null)
                response.msg = ((Exception) e).getMessage();
            return response;
        }

        ((Exception) e).printStackTrace();
        return new CommonResponse(-1, "服务器:鬼知道发生了什么,( 错误提示:" + ((Exception) e).getMessage());
    }
}
